...
__global__ void kernel0(CSRMatrix d_csrA, CSCMatrix d_cscB, unsigned int* d_rows, unsigned int* d_cols, float* d_vals, unsigned int* d_nnzCounter) {
    ...
    for (int idxA = d_csrA.rowPtrs[row]; idxA < d_csrA.rowPtrs[row + 1]; idxA++) {
        ...
        for (int idxB = d_cscB.colPtrs[a_col]; idxB < d_cscB.colPtrs[a_col + 1]; idxB++) {
            int b_col = d_cscB.rowIndices[idxB]; //CSC stores rows as rows
...
void spmspm_gpu0(COOMatrix* cooMatrix1, CSRMatrix* csrMatrix1, CSCMatrix* cscMatrix1, COOMatrix* cooMatrix2, CSRMatrix* csrMatrix2, CSCMatrix* cscMatrix2, COOMatrix* cooMatrix3, unsigned int numRows1, unsigned int numRows2, unsigned int numCols2, unsigned int numNonzeros1, unsigned int numNonzeros2) {
    ...
    cooMatrix3->rowIndices = (unsigned int*)malloc(h_nnz * sizeof(unsigned int));
    cooMatrix3->colIndices = (unsigned int*)malloc(h_nnz * sizeof(unsigned int));
    cooMatrix3->values = (float*)malloc(h_nnz * sizeof(float));

    cudaMemcpy(cooMatrix3->rowIndices, d_rows, h_nnz * sizeof(unsigned int), cudaMemcpyDeviceToHost);
    cudaMemcpy(cooMatrix3->colIndices, d_cols, h_nnz * sizeof(unsigned int), cudaMemcpyDeviceToHost);
...
#define MAX_COLS 1024  //assume an upper bound for columns

__global__ void kernel1(CSRMatrix d_csrA, CSCMatrix d_cscB, unsigned int* d_rows, unsigned int* d_cols, float* d_vals, unsigned int* d_nnzCounter, int numColsB) {
    int row = blockIdx.x * blockDim.x + threadIdx.x;
    if (row >= d_csrA.numRows) return;

    //value accumulation per column
    ...
    //non-zero entries to global memory
...
void spmspm_gpu1(COOMatrix* cooMatrix1, CSRMatrix* csrMatrix1, CSCMatrix* cscMatrix1, COOMatrix* cooMatrix2, CSRMatrix* csrMatrix2, CSCMatrix* cscMatrix2, COOMatrix* cooMatrix3, unsigned int numRows1, unsigned int numRows2, unsigned int numCols2, unsigned int numNonzeros1, unsigned int numNonzeros2) {
    ...
    int blockSize = 256;
    int numBlocks = (numRows1 + blockSize - 1) / blockSize;
    kernel1<<<numBlocks, blockSize>>>(*csrMatrix1, *cscMatrix2, d_rows, d_cols, d_vals, d_nnzCounter, numCols2);
    cudaDeviceSynchronize();
